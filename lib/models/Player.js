...
function Player(data, listener, system) {
  ...
  function notificationHandler(uuid, data) {
    ...
    // this isn't working... I think it should be surroundmode (all lower case)
    if (data.surroundmode) {
      _this._state.equalizer.surroundMode = parseInt(data.surroundmode.val);
    }
